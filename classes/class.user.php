...
class User
{
	...
	// updated update_user function to update user details, where the user has the option to update the email only, or the password, and the account status
	public function update_user($lastname, $firstname, $access, $id, $email = null, $password = null, $status = null)
	{
		...
		// SQL query
		$sql = "UPDATE tbl_users SET user_firstname=:user_firstname, user_lastname=:user_lastname, user_date_updated=:user_date_updated, user_time_updated=:user_time_updated, user_access=:user_access";

		// parameter variable for executing later
		$params = array(':user_firstname' => $firstname, ':user_lastname' => $lastname, ':user_date_updated' => $NOW, ':user_time_updated' => $NOW, ':user_access' => $access, ':user_id' => $id);
	...
	{
		// SQL query for updating the user table from the db_wbapp database
		$sql = "UPDATE tbl_users SET user_status=:user_status WHERE user_id=:user_id";
		// prepare statement for protection from SQL Injections
		$q = $this->conn->prepare($sql);
		$q->execute(array(':user_status' => '0', ':user_id' => $id));
		return true;
	}
	...
	public function activate_user($id)
	{
		$sql = "UPDATE tbl_users SET user_status=:user_status WHERE user_id=:user_id";
		$q = $this->conn->prepare($sql);
		$q->execute(array(':user_status' => '1', ':user_id' => $id));
		return true;
	}
